...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		/**
		 * В каталоге один и тот же элемент может быть в разных разделах.
		 * Но URL в разделе у него всегда такой в каком разделе он лежит
		 */
		$rsElement = CIBlockElement::GetByID($arItem["ID"]);

		if($arElementItem = $rsElement->Fetch())
		{
			if($arElementItem["IBLOCK_SECTION_ID"] <> "")
			{
				$arSort = array();
				$arFilter = array(
					"ID" => $arElementItem["IBLOCK_SECTION_ID"],
					"IBLOCK_ID" => 1
				);
				$arSelect = array(
					"ID",
				...
				if($arSectionItem = $rsSection->Fetch())
				{
					...
					$arSectionPath = array();
					while($arSectionItemPath = $rsNav->GetNext())
					{
						$arSectionPath[] = $arSectionItemPath;
					}

					$arSectionPath = array_pop($arSectionPath);

					//if($USER->isAdmin())
					//{
					//	//echo "<pre>"; var_dump($arSectionPath); echo "</pre>";
					//}
					...
					$arNewItemsList[$key]['DETAIL_PAGE_URL'] = str_replace("//","/",implode('/', $arUrl).'/');;

					//if($USER->isAdmin())
					//{
					//	echo "<pre>"; var_dump($arResult["ITEMS"][$key]['DETAIL_PAGE_URL']); echo "</pre>";
					//}
				}
			}
		}
